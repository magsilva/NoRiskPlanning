...
<div class="location">
	...
	<xsl:attribute name="href">
     ...
     Permissions</a> / Create New Permission To Room
</div>
...
<form action="adm_permissions_room_new.php" method="POST">
...
<tr>
	<th align="left"><label for="slave_id">Slave Room: </label></th>
	<td align="left">
		...
		<xsl:variable name="cur_room" select="/nrp/permission/permission.slave/@id"/>
			<option>
				<xsl:attribute name="value">
					<xsl:value-of select="@id"/>
				</xsl:attribute>
				<xsl:if test="@id = $cur_room">
					<xsl:attribute name="selected">selected</xsl:attribute>
				</xsl:if>
...
<br/><input type="submit" class="button" name="create" value="Create New Permission To Room"/>
