...
<xsl:variable name="documents_url" select="concat('groups_documents.php?sess_id=', $session_id)" />
...
<a>
<xsl:attribute name="href">
	<xsl:value-of select="$documents_url"/>&amp;group_id=<xsl:value-of select="/nrp/group/@id"/>
</xsl:attribute>
Documents</a> /
...
<p class="big_head">
Are you sure you want to remove the document "<xsl:value-of select="/nrp/document/document.description"/>" ?
</p>


<form action="groups_documents_remove.php" method="post">
<p class="big_head">
<input type="hidden" name="document_id">
	<xsl:attribute name="value"><xsl:value-of select="/nrp/document/@id"/></xsl:attribute>
</input>
