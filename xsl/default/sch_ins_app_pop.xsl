...
<head>
	<title>NRP - New Appointment Scheduling</title>
...
<div class="al_center">
<form action="sch_ins_app.php" method="POST">
<table summary ="Appointment Insertion" class="data" cellspacing="1">
<tr>
...
<xsl:if test="$master_sess != 0">
	...
	<label for="ins_at_master">Insert this appointment at the Owner's Schedule</label>
</xsl:if>

<input type="hidden" name="is_pop" value="1"/>

<br/><input type="submit" class="button" name="submit_ins" value="Insert"/>
- <input type="button" class="button" name="cancelar" value="Cancel" onclick="javascript:window.close()"/>
<br/><br/><input type="submit" class="button" name="switch" value="Switch to Weekly Appointment"/>
</form>
</div>
