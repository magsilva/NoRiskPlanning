...
<xsl:value-of select="/nrp/group/group.name"/></a> / Invite New Member
</div>
...
<p class="big_head">
To invite a new member to the group, choose the user you want to invite, and
press 'Invite':
</p>



<form action="groups_members_new.php" method="post">
<select name="member_id">
<xsl:for-each select="/nrp/person">
<xsl:sort select="person.name"/>
<xsl:variable name="person_id" select="@id"/>
<xsl:if test="count(/nrp/group/group.member[@id = $person_id]) = 0">
	<option>
...
<input type="submit" class="button" name="insert" value="Invite"/>
</form>
<br/>
</div>
