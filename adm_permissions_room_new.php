...
if (Validate_Session($complete_sess_id, $_SERVER['REMOTE_ADDR'], $bd))
{
	...
	// If the submit button to create the unit was pressed
	{
		...
		$room = List_Rooms($permissions[0][7], '', '', '', '', $bd);
		$permissions[0][8] = $room[0][1];
		$permissions[0][9] = 'room';
		...
		{
			case 0: $error[$num_errors++] = "A Slave Room must be chosen"; break;
		...
		// If only one of the options was chosen
		{
			$current = List_Permissions('', $permissions[0][1], $permissions[0][3],
				$permissions[0][5], $permissions[0][7], 'room', $bd);
			if ($current)
