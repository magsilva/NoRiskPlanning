...
if (Validate_Session($complete_sess_id, $_SERVER['REMOTE_ADDR'], $bd))
{
	...
	{
		$apps = List_Appointments($account_id, 1, $group_id , '', '', '', '', '', $_POST['app_id'], '',
			$cfg['time'], $array_type, $array_color, $array_image, $cfg['days'], $bd);
		if (!$is_pop)
			$result_xsl = "xsl/" . $default_xsl . "/groups_sch_del_app1.xsl";
		else
			$result_xsl = "xsl/" . $default_xsl . "/groups_sch_del_app_pop1.xsl";
	...
	// If the submit button to confirm the appointment deletion was pressed
	{
		...
		{
			Group_Delete_Appointment($_POST['app_id'], $account_id, $bd);
	...
	{
		$apps =	List_Appointments($account_id, 1, $group_id, '' , '', '', '', '', '', '', $cfg['time'],
		      $array_type, $array_color, $array_image, $cfg['days'], $bd);
		$result_xsl = "xsl/" . $default_xsl . "/groups_sch_del_app.xsl";
	}
}
else
{
